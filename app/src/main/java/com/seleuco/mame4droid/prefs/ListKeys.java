		...
		if (keyLabel != null) {
			...
			if (InputHandler.keyMapping[position] != -1 /*&& InputHandler.keyMapping[position] > 0*/)
			{
				textView2.setText(String.format("%d : %s"
						, InputHandler.getDeviceIdFromKeyCodeWithDeviceID(InputHandler.keyMapping[position])
						,  ListKeys.androidKeysLabels[ InputHandler.getKeyCodeFromKeyCodeWithDeviceID(InputHandler.keyMapping[position]) ]
						)
		        );				
